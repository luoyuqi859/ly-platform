        ...
        <el-table v-loading="loading" :data="hostList">
            ...
            <el-table-column label="状态">
                <template #default="{ row }">
                    <el-tag type="info" v-if="row.loading">执行中</el-tag>
                    <el-tag :type="!row.loading && row.alive ? 'success' : 'danger'" v-else>
                        {{ !row.loading && row.alive ? '在线' : '离线' }}
                    </el-tag>
            ...
            <el-table-column label="操作" width="230" class-name="fixed-width">
                <template #default="scope">
                    ...
                    <el-tooltip content="xxx" placement="top">
                        <el-button link type="primary" icon="Open" :disabled="!scope.row.alive"
                            @click="executeStep(scope)">xxx</el-button>
                    </el-tooltip>
                </template>
            </el-table-column>
        </el-table>
